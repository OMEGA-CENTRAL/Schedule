import pathlib, time, telebot, os, filecmp, shutil
from formationData import *
from peewee import *
from threading import Thread
from telebot import types 

bot = telebot.TeleBot("6594844531:AAHXLcp5GghLXXq-pUEBH9JlN4cs0p8Fb-o")
...
def getUser(id):
    for user in User.select():
        if(user.id == str(id)):
            data = {'id': user.id, 'sheduleType': user.sheduleType, 'sheduleName': user.sheduleName}
            return data
...
def sendShedule(id):
    ...
    try:
        ...
        bot.send_message(id, text= "Расписание на следуйщий день не найдено 🤔" .format(id))
...
def telegramBot():
    ...
    def startMessage(message):
        ...
        if(getUser(message.from_user.id) == 0): 
            User.create(id = str(message.from_user.id), sheduleType = 'unknown', sheduleName = 'unknown')
